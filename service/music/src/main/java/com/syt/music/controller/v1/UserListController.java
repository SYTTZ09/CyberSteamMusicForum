...
public class UserListController {
    ...
    @PostMapping("/all/{id}")
    public Response<List<MusicVO>> allList(@PathVariable Integer id, @RequestBody LoadMoreRequest request) {
        return userListService.allList(id, request);
    }

    @PostMapping("/like/{id}")
    public Response<List<MusicVO>> likeList(@PathVariable Integer id, @RequestBody LoadMoreRequest request) {
        return userListService.likeList(id, request);
    }

    @PostMapping("/public/{id}")
    public Response<List<MusicVO>> publicList(@PathVariable Integer id, @RequestBody LoadMoreRequest request) {
        return userListService.publicList(id, request);
    }

    @PostMapping("/private/{id}")
    public Response<List<MusicVO>> privateList(@PathVariable Integer id, @RequestBody LoadMoreRequest request) {
